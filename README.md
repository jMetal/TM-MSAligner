...
### Runing TM-MSAligner to align a custom TMPs dataset
...
* dataDirectory: The path of the data directory. This directory must contain the file with the protein sequences, with the extension `.3line`, this file must be generated using (generated by ..) and the Pre-Computed alignments in fasta format(`.fasta` extension) performed by state-of-the-art of MSA software used to generate the Initial population of the algorithm.
...
An example to align the custom TMPs dataset `msl` in the folder `data/custom_tests`,
with 25000 evaluations of the algorithm, a population size of 100, using 4 cores and plotting the ParetoFront each 500 evaluations, you have to run this command:
...
Furthermore, we have added the `refName_predicted_topologies.3line` for each BAliBASE-ref7 instance, saved in the `data/benchmarks/ref7` folder.
...
* 
* 
* outputFolder = Directory with the TM-MSAligner results, the path is joined as follows "data/pruebas/ref7/" + refName + "/test" + System.currentTimeMillis() +"/" ;
